...
We decided, from the start of the project, to keep a structured knowledge base in `docs/` instead of leaving knowledge scattered across code, PRs, and people's heads. It has parts with distinct roles: `CONTEXT.md` (domain glossary), `docs/adr/` (decisions and their whys), `docs/system/` (living technical docs — what the code does today), `docs/plans/` (plans for what we're going to do), `docs/pending/` (loose ends to revisit), and `docs/learnings/` (mistakes already made, not to repeat).
